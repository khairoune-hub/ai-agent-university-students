...
export async function generateReply(input: AiReplyInput): Promise<string> {
  ...
  const STYLE =
    ...
    '- عند استعمال أداة search_programs: ابدأ بسطر ملخّص (إجمالي عدد المؤسسات وكم منها مدارس عليا)، ' +
    'ثم اذكر كل المؤسسات التي أعادتها الأداة مصنّفة حسب الفئة بعناوينها العربية، ' +
    'وابدأ دائمًا بفئتَي «المدارس الوطنية العليا» و«المدارس العليا للأساتذة» إن وُجدتا. ' +
    'سطر قصير لكل مؤسسة بالشكل «الاسم — المعدل الأدنى». لا تحذف أي مؤسسة ولا تخترع غيرها.';
